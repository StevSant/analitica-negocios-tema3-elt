...
-- --- Productos -------------------------------------------------------------
CREATE OR REPLACE VIEW `hackiaton-ia.staging.stg_productos` AS
SELECT
  ...
  TRIM(marca)                            AS marca
FROM `hackiaton-ia.raw.productos`
...
-- --- Clientes --------------------------------------------------------------
CREATE OR REPLACE VIEW `hackiaton-ia.staging.stg_clientes` AS
SELECT
...
FROM `hackiaton-ia.raw.clientes`
WHERE customer_id IS NOT NULL
QUALIFY ROW_NUMBER() OVER (PARTITION BY TRIM(customer_id) ORDER BY TRIM(customer_id)) = 1;

-- --- Ventas (hechos) -------------------------------------------------------
CREATE OR REPLACE VIEW `hackiaton-ia.staging.stg_ventas` AS
SELECT
...
-- --- Verificación ----------------------------------------------------------
-- SELECT * FROM `hackiaton-ia.staging.stg_ventas` LIMIT 20;
