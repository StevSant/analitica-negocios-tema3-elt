...
-- (patrón estrella). Reemplaza `august-tower-470819-s6`.
-- =============================================================================

-- KPI 1 — Ventas netas por mes (tendencia temporal, analítica descriptiva)
CREATE OR REPLACE VIEW `august-tower-470819-s6.analytics.kpi_ventas_mensuales` AS
SELECT
  f.anio,
  f.mes,
  f.nombre_mes,
...
FROM `august-tower-470819-s6.analytics.fact_ventas` v
JOIN `august-tower-470819-s6.analytics.dim_fecha`   f USING (fecha)
...
CREATE OR REPLACE VIEW `august-tower-470819-s6.analytics.kpi_margen_categoria` AS
SELECT
  p.categoria,
  ROUND(SUM(v.monto_neto), 2)                              AS ventas_netas,
...
FROM `august-tower-470819-s6.analytics.fact_ventas`   v
JOIN `august-tower-470819-s6.analytics.dim_producto`  p USING (product_id)
GROUP BY p.categoria
ORDER BY margen DESC;

-- KPI 3 — Desempeño por canal de venta
CREATE OR REPLACE VIEW `august-tower-470819-s6.analytics.kpi_ventas_canal` AS
SELECT
  canal,
  COUNT(*)                    AS num_ordenes,
...
CREATE OR REPLACE VIEW `august-tower-470819-s6.analytics.kpi_ventas_segmento` AS
SELECT
  c.segmento,
  COUNT(DISTINCT v.customer_id) AS clientes_activos,
  ROUND(SUM(v.monto_neto), 2)   AS ventas_netas
FROM `august-tower-470819-s6.analytics.fact_ventas`  v
JOIN `august-tower-470819-s6.analytics.dim_cliente`  c USING (customer_id)
GROUP BY c.segmento
ORDER BY ventas_netas DESC;

-- KPI 5 — Top 10 clientes por valor
CREATE OR REPLACE VIEW `august-tower-470819-s6.analytics.kpi_top_clientes` AS
SELECT
  c.customer_id,
  c.nombre,
  c.ciudad,
...
-- SELECT * FROM `august-tower-470819-s6.analytics.kpi_ventas_mensuales`;
-- SELECT * FROM `august-tower-470819-s6.analytics.kpi_margen_categoria`;
